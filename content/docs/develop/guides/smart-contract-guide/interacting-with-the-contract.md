...
```sh
xplad tx wasm store artifacts/my_first_contract.wasm --from test1 --chain-id=cube_37-1 --gas=auto --fees=100000axpla --broadcast-mode=block
```
Or, if you are on an arm64 machine:

```sh
xplad tx wasm store artifacts/my_first_contract-aarch64.wasm --from test1 --chain-id=cube_37-1 --gas=auto --fees=100000axpla --broadcast-mode=block
```
...
```sh
xplad tx wasm instantiate 1 '{"count":0}' --from test1 --chain-id=cube_37-1 --fees=10000axpla --gas=auto --broadcast-mode=block
```
...
```sh
xplad tx wasm execute xpla18vd8fpwxzck93qlwghaj6arh4p7c5n896xzem5 '{"reset":{"count":5}}' --from test1 --chain-id=cube_37-1 --fees=1000000axpla --gas=auto --broadcast-mode=block
```
...
```sh
xplad tx wasm execute xpla18vd8fpwxzck93qlwghaj6arh4p7c5n896xzem5 '{"increment":{}}' --from test1 --chain-id=cube_37-1 --gas=auto --fees=1000000axpla --broadcast-mode=block
```
